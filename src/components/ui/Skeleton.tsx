...
function Skel({ className, style }: { className?: string; style?: React.CSSProperties }) {
  return <div className={cn('animate-pulse rounded-lg bg-border/60', className)} style={style} />;
}
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-3">
        {Array.from({ length: 5 }).map((_, i) => (
          <Skel key={i} className="h-24 flex-1 rounded-lg" />
        ))}
      </div>
...
export function SkeletonVideo() {
  return (
    <div className="space-y-4 rounded-lg border border-border bg-[#1A1A18] p-4 shadow-sm">
      <Skel className="aspect-[9/16] max-h-[560px] w-full rounded-lg bg-white/10 md:aspect-video" />
