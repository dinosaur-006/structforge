...
export function VideoPlayer({
    ...
    return (
      <div className="rounded-xl border border-border/60 bg-white p-4 shadow-sm">
        <video
          ...
          className="aspect-[9/16] max-h-[560px] w-full rounded-xl bg-[#FAFAF9] object-contain md:aspect-video"
          src={src}
          controls
  ...
  if (isRendering) {
    return (
      <div className="rounded-xl border border-primary/20 bg-white p-4 shadow-sm">
        <div className="grid aspect-[9/16] max-h-[560px] place-items-center rounded-xl bg-[#FAFAF9] md:aspect-video">
          <div className="text-center space-y-4">
            ...
            {renderWarnings && renderWarnings.length > 0 && (
              <div className="mt-3 max-h-32 overflow-y-auto space-y-0.5 text-left">
                {renderWarnings.filter(w => w.includes(':') && (w.includes('hook') || w.includes('pain') || w.includes('product') || w.includes('proof') || w.includes('cta'))).slice(-8).map((w, i) => (
                  <p key={i} className={`text-[10px] font-mono ${w.startsWith('✓') ? 'text-success' : 'text-text-muted'}`}>
                    {w.startsWith('✓') ? '✅ ' : '🔄 '}{w.replace('✓ ', '')}
                  </p>
                ))}
              </div>
            )}
  ...
  return (
    <div className="rounded-xl border border-border/60 bg-white p-4 shadow-sm">
        ...
        {hasDraftSegments && draftSegments.length > 0 ? (
          <div className="w-full h-full flex flex-col p-4">
            <div className="flex items-center gap-2 mb-3">
              <span className="inline-flex h-2 w-2 rounded-full bg-[#C8843C] animate-pulse" />
              <span className="text-xs font-medium text-[#C8843C]">
                AI 蓝图草稿 — 以下提示词可直接用于文生视频
              </span>
            </div>
            <div className="flex-1 overflow-y-auto space-y-2">
              {draftSegments.slice(0, 3).map((seg) => (
                <div key={seg.id} className="rounded-md border border-[#C8843C]/15 bg-white p-3">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button
                      type="button"
                      className="text-[10px] text-[#C8843C] hover:underline"
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-text-muted">点击上方 RENDER ALL 开始渲染视频</p>
          </div>
        )}
      </div>
      ...
      {hasDraftSegments && (
        <div className="mt-2 rounded-md border border-[#C8843C]/20 bg-[#C8843C]/5 px-3 py-2 flex items-center gap-3">
          <button
            type="button"
            className="flex items-center gap-1.5 text-xs text-[#C8843C] hover:text-[#B07530] transition-colors"
